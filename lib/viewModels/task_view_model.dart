...
class TaskViewModel extends ChangeNotifier {
  ...
  Future<bool> createTask(
    ...
    if (response is Success) {
      ...
      // Handle null or empty date properly before splitting
      List<String> date = [];
      if (generatedDate != null && generatedDate.isNotEmpty) {
        date = generatedDate.split("-");
      }

      // Handle case where date splitting might fail or date is not valid
      if (date.isNotEmpty) {
        // Rearranging the date
        date.insert(0, date.removeAt(2)); // Year -> Day
        date.insert(1, date.removeAt(2)); // Month -> Year
        taskData.createdDate = date.join("-"); // Reassign formatted date
      } else {
        // Handle cases where the date is not valid or null
        taskData.createdDate =
            "Invalid Date"; // Default value or error handling
      }
